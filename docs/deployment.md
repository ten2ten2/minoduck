...
2. `MIGRATION_DATABASE_URL` 使用迁移账户。另建 `minoduck_app`，属性为 `NOSUPERUSER NOBYPASSRLS NOCREATEDB NOCREATEROLE`，运行进程的 `DATABASE_URL` 使用此角色。不要把数据库管理员连接直接用于 API/Worker。
...
5. `/healthz` 检查进程，`/readyz` 检查数据库连接；生产启动会拒绝绕过 RLS 的运行角色。
...
生产 session 为 `__Host-md_session`，带 HttpOnly、Secure、SameSite=Lax；前端不接触服务端密钥。
...
配置 `STRIPE_SECRET_KEY`、`STRIPE_WEBHOOK_SECRET` 和四个 Price ID。Webhook 使用 SDK 对应的 API 版本 `2026-08-26.dahlia`，选择 **snapshot events**，端点为 `https://api.minoduck.ai/api/v1/webhooks/stripe`，订阅：

- `checkout.session.completed`
- `customer.subscription.created`、`customer.subscription.updated`、`customer.subscription.deleted`
- `invoice.paid`、`invoice.payment_failed`

创建独立 Customer Portal configuration，允许更新支付方式、查看账单，禁用 Portal 订阅换档，并配置 `STRIPE_PORTAL_CONFIGURATION`。套餐变更由应用处理，确保立即升级、期末降级的规则一致。
...
4. 手动运行 `Deploy Cloudflare`。Workflow 检查语言键、重定向和类型，并构建发布官网、控制台与根域 Worker。
5. 验证 session/CSRF、私有资源 no-store/noindex；访问日志排除认证 query、cookie、邮件、凭据和账单正文，只记录脱敏的 request ID、route 与 status。
