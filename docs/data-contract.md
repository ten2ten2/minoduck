...
L1 只计算当前实现明确支持的文本用量与历史价格，不把 Provider 的总成本端点强行解释成文本费用明细。`native-cost` 仍作为可靠的财务 Actual 证据保存，但不会因为其总额与文本计算结果接近就标记为 `matched` 或 `within_tolerance`。要得到可比较的 L1 结论，需要一个独立、明确只覆盖可比费用的 source scope，并满足完整 coverage、价格版本与所需计费维度。
...
错误为 `{ "error": { "code": "...", "message_key": "errors....", "retryable": false, "request_id": "..." } }`。金额字符串；未知值用 null，与 `"0"` 区分。JSON 请求只接受一个顶层值，未知字段或尾随第二个 JSON 值均视为无效请求。402 为套餐门槛；403 为权限/CSRF；404 隐藏非成员资源；409 为版本/状态冲突；503 为暂不可用。
